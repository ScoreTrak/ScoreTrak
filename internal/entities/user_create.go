...
func (uc *UserCreate) AddCompetitions(c ...*Competition) *UserCreate {
	...
	return uc.AddCompetitionIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.CompetitionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
...
func (u *UserUpsertOne) UpdateNewValues() *UserUpsertOne {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		...
		if _, exists := u.create.mutation.OryID(); exists {
			s.SetIgnore(user.FieldOryID)
		}
	}))
	return u
}
...
func (u *UserUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entities: missing options for UserCreate.OnConflict")
	}
...
func (u *UserUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entities: UserUpsertOne.ID is not supported by MySQL driver. Use UserUpsertOne.Exec instead")
	}
...
func (u *UserUpsertBulk) UpdateNewValues() *UserUpsertBulk {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		for _, b := range u.create.builders {
			...
			if _, exists := b.mutation.OryID(); exists {
				s.SetIgnore(user.FieldOryID)
			}
		}
	}))
...
func (u *UserUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entities: OnConflict was set for builder %d. Set it on the UserCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entities: missing options for UserCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
