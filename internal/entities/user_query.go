...
func (uq *UserQuery) QueryCompetitions() *CompetitionQuery {
	query := (&CompetitionClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCompetitions: uq.withCompetitions.Clone(),
...
func (uq *UserQuery) WithCompetitions(opts ...func(*CompetitionQuery)) *UserQuery {
	query := (&CompetitionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCompetitions = query
	return uq
}
...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	...
	if user.Policy == nil {
		return errors.New("entities: uninitialized user.Policy (forgotten import entities/runtime?)")
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCompetitions; query != nil {
		if err := uq.loadCompetitions(ctx, query, nodes,
...
func (uq *UserQuery) loadCompetitions(ctx context.Context, query *CompetitionQuery, nodes []*User, init func(*User), assign func(*User, *Competition)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*User)
	nids := make(map[string]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.CompetitionsTable)
		s.Join(joinT).On(s.C(competition.FieldID), joinT.C(user.CompetitionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.CompetitionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.CompetitionsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
