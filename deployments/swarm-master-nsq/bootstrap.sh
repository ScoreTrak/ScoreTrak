#!/bin/bash
docker network create -d overlay --attachable swarm-master-nsq_cockroachdb
docker network create -d overlay --attachable swarm-master-nsq_nsq

docker service create \
--replicas 1 \
--name swarm-master-nsq_cockroachdb-1 \
--hostname cockroachdb-1 \
--network name=swarm-master-nsq_cockroachdb,alias=cockroach,alias=cockroachdb-1 \
...
docker service create \
--replicas 1 \
--name swarm-master-nsq_cockroachdb-2 \
--hostname cockroachdb-2 \
--network name=swarm-master-nsq_cockroachdb,alias=cockroach,alias=cockroachdb-2 \
...
docker service create \
--replicas 1 \
--name swarm-master-nsq_cockroachdb-3 \
--hostname cockroachdb-3 \
--network name=swarm-master-nsq_cockroachdb,alias=cockroach,alias=cockroachdb-3 \
...
docker run -it --rm --network=swarm-master-nsq_cockroachdb cockroachdb/cockroach:v20.1.2 init --host=cockroachdb-1 --insecure

sleep 2s

docker run -it --rm --network=swarm-master-nsq_cockroachdb cockroachdb/cockroach:v20.1.2 sql --execute="CREATE DATABASE IF NOT EXISTS scoretrak;" --insecure --host cockroachdb-1
...
--replicas 3 \
--publish 33333:33333 \
--name swarm-master-nsq_scoretrak \
--constraint node.role==manager \
--hostname scoretrak \
