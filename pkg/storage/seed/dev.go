...
func DevSeed(cfg *config.Config, entitiesClient *entities.Client, authClient *client.APIClient) {
	...
	authenticatedCtx := user.NewContext(ctx, i)
	...
	if !cfg.Prod {
		...
		services := entitiesClient.Service.CreateBulk(
			entitiesClient.Service.Create().SetName("lamp").SetDisplayName("LAMP Stack").SetCompetition(comps[0]),
			entitiesClient.Service.Create().SetName("db").SetDisplayName("DB Stack").SetCompetition(comps[0]),
			entitiesClient.Service.Create().SetName("ftp").SetDisplayName("FTP Stack").SetCompetition(comps[0]),
			entitiesClient.Service.Create().SetName("imap").SetDisplayName("IMAP Stack").SetCompetition(comps[0]),
			entitiesClient.Service.Create().SetName("lamp").SetDisplayName("LAMP Stack").SetCompetition(comps[1]),
			entitiesClient.Service.Create().SetName("db").SetDisplayName("DB Stack").SetCompetition(comps[1]),
			entitiesClient.Service.Create().SetName("ftp").SetDisplayName("FTP Stack").SetCompetition(comps[1]),
			entitiesClient.Service.Create().SetName("imap").SetDisplayName("IMAP Stack").SetCompetition(comps[1]),
		).SaveX(authenticatedCtx)

		for _, service := range services {
			for _, team := range teams {
				host := entitiesClient.Host.Create().SetAddress("1.1.1.1").SetTeam(team).SaveX(authenticatedCtx)
				hostservice := entitiesClient.HostService.Create().SetTeam(team).SetHost(host).SetName(service.Name).SetDisplayName(service.DisplayName).SaveX(authenticatedCtx)
				_ = entitiesClient.Property.Create().SetTeam(team).SetHostservice(hostservice).SetKey("a").SetValue("v").SaveX(authenticatedCtx)
			}
		}
	}
