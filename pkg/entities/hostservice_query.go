...
func (hsq *HostServiceQuery) Order(o ...hostservice.OrderOption) *HostServiceQuery {
...
// QueryHostservicereport chains the current query on the "hostservicereport" edge.
func (hsq *HostServiceQuery) QueryHostservicereport() *HostServiceReportQuery {
	query := (&HostServiceReportClient{config: hsq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hsq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hsq *HostServiceQuery) Clone() *HostServiceQuery {
	...
	return &HostServiceQuery{
		config:                hsq.config,
		ctx:                   hsq.ctx.Clone(),
		order:                 append([]hostservice.OrderOption{}, hsq.order...),
		...
		withHostservicereport: hsq.withHostservicereport.Clone(),
...
// WithHostservicereport tells the query-builder to eager-load the nodes that are connected to
// the "hostservicereport" edge. The optional arguments are used to configure the query builder of the edge.
func (hsq *HostServiceQuery) WithHostservicereport(opts ...func(*HostServiceReportQuery)) *HostServiceQuery {
	query := (&HostServiceReportClient{config: hsq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hsq.withHostservicereport = query
	return hsq
}
...
func (hsq *HostServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*HostService, error) {
	...
	if query := hsq.withHostservicereport; query != nil {
		if err := hsq.loadHostservicereport(ctx, query, nodes, nil,
			func(n *HostService, e *HostServiceReport) { n.Edges.Hostservicereport = e }); err != nil {
			return nil, err
		}
	}
...
func (hsq *HostServiceQuery) loadChecks(ctx context.Context, query *CheckQuery, nodes []*HostService, init func(*HostService), assign func(*HostService, *Check)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(check.FieldHostServiceID)
	}
	query.Where(predicate.Check(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(hostservice.ChecksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "host_service_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (hsq *HostServiceQuery) loadProperties(ctx context.Context, query *PropertyQuery, nodes []*HostService, init func(*HostService), assign func(*HostService, *Property)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(property.FieldHostServiceID)
	}
	query.Where(predicate.Property(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(hostservice.PropertiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (hsq *HostServiceQuery) loadHostservicereport(ctx context.Context, query *HostServiceReportQuery, nodes []*HostService, init func(*HostService), assign func(*HostService, *HostServiceReport)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*HostService)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(hostservicereport.FieldHostServiceID)
	}
	query.Where(predicate.HostServiceReport(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(hostservice.HostservicereportColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
