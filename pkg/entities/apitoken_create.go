...
func (atc *ApiTokenCreate) SetRoleID(s string) *ApiTokenCreate {
	atc.mutation.SetRoleID(s)
	return atc
}
...
func (atc *ApiTokenCreate) SetUserID(s string) *ApiTokenCreate {
...
func (atc *ApiTokenCreate) check() error {
	...
	if _, ok := atc.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role_id", err: errors.New(`entities: missing required field "ApiToken.role_id"`)}
	}
	if _, ok := atc.mutation.ExpiredAt(); !ok {
	...
	if _, ok := atc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`entities: missing required field "ApiToken.user_id"`)}
	}
...
func (atc *ApiTokenCreate) createSpec() (*ApiToken, *sqlgraph.CreateSpec) {
	...
	if value, ok := atc.mutation.RoleID(); ok {
		_spec.SetField(apitoken.FieldRoleID, field.TypeString, value)
		_node.RoleID = value
	}
...
func (u *ApiTokenUpsertOne) UpdateNewValues() *ApiTokenUpsertOne {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		...
		if _, exists := u.create.mutation.RoleID(); exists {
			s.SetIgnore(apitoken.FieldRoleID)
		}
		if _, exists := u.create.mutation.UserID(); exists {
			s.SetIgnore(apitoken.FieldUserID)
		}
	}))
	return u
}
