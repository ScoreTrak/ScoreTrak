...
func NewCompetitionServ(str *util.Store, cfg config.Config) Serv {
...
func (svc *competitionServ) LoadCompetition(ctx context.Context, comp *competition.Competition) error {
	...
	// }
	// for i := range comp.WorkerGroups {
	// 	errAgr = append(errAgr, svc.Store.WorkerGroup.Upsert(ctx, comp.WorkerGroups[i]))
	// }
...
func (svc *competitionServ) FetchCoreCompetition(ctx context.Context) (*competition.Competition, error) {
	...
	// 	return nil, wrapError(err, fetch)
	// }
	// workerGroups, err := svc.Store.WorkerGroup.GetAll(ctx)
	// if svc.Config.Use != "none" {
	// 	for i := range workerGroups {
	// 		workerGroups[i].Enabled = &fls
	// 	}
	// }
	// if err != nil {
	// 	return nil, wrapError(err, fetch)
	...
	// 	return nil, wrapError(err, fetch)
	// }
	// return &competition.Competition{Config: cnf, Teams: teams, HostGroups: hostsGroup, Hosts: hosts, WorkerGroups: serviceGroups, Services: services, Properties: properties, Policy: pol, Users: users}, nil
	return &competition.Competition{}, nil
}
