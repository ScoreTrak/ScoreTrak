...
func (f *FTP) Execute(e exec.Exec) (passed bool, l string, err error) {
	...
	c, err := ftp.Dial(fmt.Sprintf("%s:%s", e.HostAddress, f.Port), dialOptions) // For passive FTP allow Data Channel Port Range. In addition, Allow FTP as an APP in windows firewall, and allow port 20, 21, 1024-65535
	if err != nil {
