...
// func (d Scheduler) refreshDsync() error {
// 	var tm time.Time
...
// func NewRunner(db *gorm.DB, q queue.WorkerQueue, r *util.Store, staticConfig config.Config) *Scheduler {
// 	return &Scheduler{
// 		db: db, q: q, r: r, staticConfig: staticConfig,
// 	}
// }

// func InitRunner(lc fx.Lifecycle, runner *Scheduler) {
// 	lc.Append(fx.Hook{
...
// func (d *Scheduler) run(configLoop *time.Ticker, scoringLoop *time.Timer, cnf *config.DynamicConfig, lastRound *round.Round) (err error) {
// 	for {
// 	runnerSelect:
// 		select {
// 		case <-configLoop.C:
...
// 				case time.Now().After(lastRound.StartedAt.Add(time.Duration(cnf.RoundDuration) * time.Second).Add(config.MinRoundDuration)):
...
// 						if lastRound.ID == delayedLastRound.ID && delayedLastRound.FinishedAt == nil {
// 							now := time.Now()
// 							// finalizes the round that failed to score
// 							err = tx.Model(delayedLastRound).Updates(round.Round{FinishedAt: &now, Note: "This round did not score!", Err: "this round is now skipped because it took to long to score. This could be due to too low round duration, dead master, or a time desync between master instances."}).Error
// 							if err != nil {
// 								return err
// 							}
// 						}
...
// func (d *Scheduler) MasterRunner() (err error) {
// 	err = d.refreshDsync()
// 	if err != nil {
// 		return err
// 	}
...
// func (d *Scheduler) durationUntilNextRound(rnd *round.Round, roundDuration uint64) time.Duration {
// 	// Start time of current round + Round Duration - Current time on database
// 	dur := rnd.StartedAt.Add(time.Duration(roundDuration) * time.Second).Sub(time.Now().Add(d.dsync))
...
// func (d Scheduler) generateScoredServices(ctx context.Context, teams []*team.Team, hostGroup []*hostgroup.HostGroup, serviceGroups []*workergroup.WorkerGroup, rnd *round.Round) []*queueing.ScoringData {
...
// 								if services[s].WorkerGroupID == servGroup.ID && *(servGroup.Enabled) {
...
// func (d Scheduler) Score(timeout time.Duration, rnd *round.Round) {
...
// 	serviceGroups, err := d.r.WorkerGroup.GetAll(ctx)
// 	if err != nil {
...
// func (d Scheduler) generateSimpleTeamReport(teams []*team.Team, hostGroup []*hostgroup.HostGroup, serviceGroups []*workergroup.WorkerGroup, rnd *round.Round, tp map[uuid.UUID]uint64) map[uuid.UUID]*report.SimpleTeam {
...
// 					}
// 					var simpSgr *report.SimpleWorkerGroup
// 					for _, sG := range serviceGroups {
// 						if sG.ID == s.WorkerGroupID {
// 							simpSgr = &report.SimpleWorkerGroup{ID: sG.ID, Name: sG.Name, Enabled: *sG.Enabled}
// 						}
// 					}

// 					sh.Services[s.ID] = &report.SimpleService{Name: s.Name, DisplayName: s.DisplayName, Hide: *s.Hide, Pause: *s.Pause, Points: points, Properties: params, PointsBoost: *s.PointsBoost, SimpleWorkerGroup: simpSgr, Weight: *s.Weight}
...
// func (d Scheduler) transformChecks(teams []*team.Team, chks []*queueing.QCheck, servicesToBeScored []*queueing.ScoringData, rnd *round.Round) []*check.Check {
...
// func (d Scheduler) finalizeRound(ctx context.Context, rnd *round.Round, note string, errStr string) {
