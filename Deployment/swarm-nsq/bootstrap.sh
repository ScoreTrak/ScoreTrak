...
docker run -it --rm --network=swarm-nsq_cockroachdb cockroachdb/cockroach:v20.1.2 sql --execute="CREATE DATABASE IF NOT EXISTS scoretrak; CREATE DATABASE IF NOT EXISTS scoretrak_web;" --insecure --host cockroach
...
docker config create scoretrak-config - < ./swarm-nsq/config.yml
...
ghcr.io/scoretrak/scoretrak/scoretrak-server:v0.1.8 ./master -config /config.yml

docker service create \
--replicas 3 \
--publish 44444:44444 \
--name swarm-nsq_scoretrakweb \
--hostname scoretrakweb \
--network swarm-nsq_nsq \
--network swarm-nsq_cockroachdb \
--replicas-max-per-node 1 \
--config src=scoretrak-config,target=/config.yml \
