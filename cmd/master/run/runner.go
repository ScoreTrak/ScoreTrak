				...
				if err != nil {
					...
					err = d.db.Transaction(func(tx *gorm.DB) error {
						...
						if lastRound.ID == delayedLastRound.ID && delayedLastRound.Finish == nil {
							now := time.Now()
							err = tx.Model(delayedLastRound).Updates(round.Round{Finish: &now, Note: "This round did not score!", Err: "this round is now skipped because it took to long to score. This could be due too low round duration, dead master, time desync between master instances."}).Error
							if err != nil {
								return err
							}
						}
